  ...
  useEffect(() => {
    const fetchPurchaser = async () => {
      ...
      try {
        ...
        if (!res.ok)
          throw new Error(responseJson.message || "Failed to fetch details");
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(apiUrl("/medicine"), { headers });
        ...
        // Data Diagnostics
        console.log(
          `[Dashboard] Loaded ${medReq.data?.length || 0} medicines and ${stockistReq.data?.length || 0} stockists.`,
        );
        if (medReq.data?.[0])
          console.log(
            `[Dashboard] Med Sample:`,
            JSON.stringify(medReq.data[0]).slice(0, 200),
          );
        if (stockistReq.data?.[0])
          console.log(
            `[Dashboard] Stockist Sample:`,
            JSON.stringify(stockistReq.data[0]).slice(0, 200),
          );
      } catch (e) {
